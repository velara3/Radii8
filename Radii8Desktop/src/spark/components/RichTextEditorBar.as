	...
	public class RichTextEditorBar extends SkinnableComponent
	{
		...
		private function applyLink(href:String, target:String = null, extendToLinkBoundary:Boolean = false, operationState:SelectionState = null):void
		{
			if (richEditableText && richEditableText.textFlow && richEditableText.textFlow.interactionManager is IEditManager)
			{
				...
				var txtLayFmt:TextLayoutFormat = richEditableText.textFlow.interactionManager.getCommonCharacterFormat();
				//Set the link
				if (operationState == null && _linkEl != null)
				{
					operationState = new SelectionState(richEditableText.textFlow, _linkEl.getAbsoluteStart(), _linkEl.getAbsoluteStart() + _linkEl.textLength);
				}
				var linkElement:LinkElement = IEditManager(richEditableText.textFlow.interactionManager).applyLink(href, target, extendToLinkBoundary, operationState);
				//Fix the formatting
				if(linkElement)
				{
					IEditManager(richEditableText.textFlow.interactionManager).clearFormatOnElement(linkElement.getChildAt(0), txtLayFmt);
				}
				var selectionEnd:int = Math.max(richEditableText.selectionActivePosition, richEditableText.selectionAnchorPosition);
				richEditableText.selectRange(selectionEnd, selectionEnd);
				IEditManager(richEditableText.textFlow.interactionManager).applyLeafFormat(txtLayFmt);
			}
		}
		...
		private function handleSelectionChange(e:FlexEvent = null):void
		{
			if (richEditableText != null)
			{
				...
				if (linkTool != null)
				{ 
					var bulletSelectionState:SelectionState = richEditableText.textFlow.interactionManager.getSelectionState();
					...
					linkTool.enabled = richEditableText.selectionAnchorPosition != richEditableText.selectionActivePosition || _linkSelected;
				}
			}
		}
