/**
 * Duplicate of Effect class but used so we can step through during debugging
 * since it was off by hundreds of lines in Flash Builder
 */
...
package com.flexcapacitor.managers
{
	...
	/**
	 *  Dispatched when the effect repeats.
	 *
	...
	/**
	 *  Dispatched when the effect updates.
	 *
	 ...
	 *  @productversion Flex 3
	 */
	public class HistoryEffect extends EventDispatcher implements IEffect
	{
		 ...
		 *  @productversion Flex 3
		 */
		public function HistoryEffect(target:Object = null)
		{
			super();
		...
		{       
			...
			if (create) 
			{
				...
				EventDispatcher(newInstance).addEventListener(EffectEvent.EFFECT_REPEAT, effectRepeatHandler);
		...
		public function deleteInstance(instance:IEffectInstance):void
		{
			...
			EventDispatcher(instance).removeEventListener(
				EffectEvent.EFFECT_REPEAT, effectRepeatHandler);
			EventDispatcher(instance).removeEventListener(
		...
		/**
		 *  This method is called when the effect instance repeats. 
		 *  If you override this method, ensure that you call the super method. 
		 *
		 *  @param event An event object of type EffectEvent.
		 *  
		 ...
		 *  @productversion Flex 3
		 */
		protected function effectRepeatHandler(event:EffectEvent):void 
		{
			if (hasEventListener(EffectEvent.EFFECT_REPEAT)) {
				dispatchEvent(event);
			}
		}
		
		/**
		 *  This method is called when the effect instance updates.
		 *  If you override this method, ensure that you call the super method. 
		 *
		 *  @param event An event object of type EffectEvent.
		 *  
		...
		{
			if (hasEventListener(EffectEvent.EFFECT_UPDATE)) {
				dispatchEvent(event);
			}
		}
	}
