...
package com.flexcapacitor.utils {
	 ...
	 * */
	public class DragManagerUtil2 extends EventDispatcher {
		
		
		public function DragManagerUtil2():void {
		...
		public function listenForDragBehavior(dragInitiator:IUIComponent, document:IDocument, event:MouseEvent, draggedItem:Object = null):void {
			...
			if (debug) {
				trace("Listening for drag behavior on:" + ClassUtils.getClassNameOrID(dragInitiator));
			}
		...
		/**
		 * Updates the drop target location string with the x and y values of the dragged item.
		 * Format is "XxY", so for example, "100x50".
		 * */
		public function updateDropTargetLocation(rootDisplayObject:DisplayObject, event:MouseEvent):void {
		...
		public function startDrag(dragInitiator:IUIComponent, application:Application, event:MouseEvent):void {
			...
			// must occur after adding to stage because of mouse enabled like properties seem to 
			// be set after creation complete or adding to stage
			removeSelectionListeners(selectionGroup);
			...
			if (debug) {
				trace("Drag Start:" + ClassUtils.getClassNameOrID(dragInitiator));
			}
		...
		private function dragEnterHandler(event:DragEvent):void {
			...
			if (debug) {
				trace("Drag Enter:" + ClassUtils.getClassNameOrID(event.target));
			}
		}
		
		protected function dragExitHandler(event:DragEvent):void {
			if (debug) {
				trace("Drag Exit:" + ClassUtils.getClassNameOrID(event.target));
			}
		}
		...
		protected function dragOverHandler(event:DragEvent):void {
			...
			if (debug) {
				if (dragData) {
					trace("Drag over: " + ClassUtils.getClassNameOrID(eventTarget) + ". Drop target: " + ClassUtils.getClassNameOrID(dragData.target));
				}
				else {
					trace("Drag over: " + ClassUtils.getClassNameOrID(eventTarget) + ". No drop target found.");
				}
			}
		...
		protected function dragDropHandler(event:DragEvent):void {
			...
			if (debug) {
				if (dragData) {
					trace("Drag drop event target: " + ClassUtils.getClassNameOrID(event.target) + ". Drop target: " + ClassUtils.getClassNameOrID(dragData.target));
				}
				else {
					trace("Drag drop event target: " + ClassUtils.getClassNameOrID(event.target) + ". No drop target found");
				}
			}
			...
			if (debug) {
				trace("drop action: " + eventDescription);
			}
		...
		protected function dragCompleteHandler(event:DragEvent):void {
			...
			removeDragDisplayObjects();
			removeGroupListeners(targetApplication);
			addSelectionListeners(selectionGroup);
		...
		protected function mouseUpHandler(event:Event):void {
			if (debug) {
				trace("Drag initiator mouse up: " + ClassUtils.getClassNameOrID(event.currentTarget));
			}
		...
		protected function removeMouseHandlers(target:IUIComponent):void {
			if (debug) {
				trace("Removing mouse up handlers: " + ClassUtils.getClassNameOrID(target));
			}
		...
		/**
		 * Create a drop indicator on non-basic layout groups
		 * */
		...
		/**
		 * Destroys drop indicator on non-basic layout groups
		 * */
		...
		/**
		 * Finds the first visual element container for the given object
		 * */
		public static function getContainerUnderPoint(event:Object, topLevelContainer:DisplayObjectContainer, adjustMouseOffset:Boolean = true, offset:Point = null):* {
			...
			if (debug) {
				trace("get container under point");
			}
		...
		*/
		/**
		 * Disables the mouse listening of the selection group
		 * */
		public function removeSelectionListeners(element:GroupBase):void {
			if (debug) {
				trace("Remove selection listeners:" + ClassUtils.getIdentifierOrName(element, true, true));
			}
			element.mouseEnabled = false;
			element.mouseChildren = false;
			//element.mouseEnabledWhereTransparent = false;
		}
		
		
		/**
		 * Enables the mouse listening of the selection group
		 * */
		public function addSelectionListeners(element:GroupBase):void {
			if (debug) {
				trace("Add selection listeners: "+ ClassUtils.getIdentifierOrName(element, true, true));
			}
			//element.mouseEnabled = true;
			//element.mouseChildren = true;
			//element.mouseEnabledWhereTransparent = true;
		}
